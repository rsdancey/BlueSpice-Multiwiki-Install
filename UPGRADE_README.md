...
- Wrapper scripts on the host only take effect if the compose file points at them. Step 1c3 repairs `docker-compose.main.yml` for wikis generated before the wrappers existed: it sets each service's `entrypoint` to the wrapper and adds the `/opt/bluespice/scripts:/scripts:ro` mount. Without it the container would still exec `/app/bin/start-web` directly and no patches would be applied.
- After the containers are back up, Step 14 runs `verify_configmanager_patches()` (`lib/verify-patches.sh`), which exercises the patched sources pulled out of the running container to confirm the Authentication tab still saves correctly — one harness per patch group, in `tests/`. The `KeyObjectInputWidget.php` check runs inside the wiki container; the two JS checks need `node`, so they use any running container that has one, or a throwaway `docker run --rm` container from a local image that does. It is non-fatal: a failure, or a check that could not run at all, is reported as an error but the upgrade is not rolled back. See the README for how to run it by hand.
